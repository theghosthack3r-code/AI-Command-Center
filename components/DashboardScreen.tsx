...
const Card: React.FC<{ children: React.ReactNode; className?: string; title?: string; }> = ({ children, className, title }) => (
    <div className={`rounded-2xl border border-border bg-card shadow-lg ${className}`}>
...
const GreetingPanel: React.FC<{ pendingApprovals: number; failedRuns: number; project: Project }> = ({ pendingApprovals, failedRuns, project }) => {
    ...
    const newMessages = 3; // Mocked
    const topPriority = MOCK_ATTENTION[0];
    const summary = project.branding?.tagline || `This project has ${pendingApprovals} pending approvals and ${failedRuns} failed runs. I suggest starting with ${topPriority.title}.`;
...
export const OverviewScreen: React.FC<{
    ...
    project: Project;
}> = ({ approvals, handleApprovalAction, setPage, project }) => {
    ...
    return (
        <div className={`grid grid-cols-12 ${densityClass}`}>
            ...
            <div className={`col-span-12 grid grid-cols-2 ${densityClass} md:grid-cols-4`}>
                <KpiCard title="Active Agents" value={projectKPIs.activeAgents} trend="+3%" trendDirection="up" />
                <KpiCard title="Pending Approvals" value={pendingApprovals.length} trend="-1" trendDirection="down" />
                <KpiCard title="Runs Today" value={projectKPIs.runsToday} trend="+12%" trendDirection="up" />
                <KpiCard title="Delivery Success" value={`${projectKPIs.deliverySuccessPct}%`} trend="-0.2%" trendDirection="down" />
            </div>
