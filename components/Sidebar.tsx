...
export const Sidebar: React.FC<{
  ...
  const getInitialOpenState = (projectId: string): string[] => {
    try {
      const storedState = localStorage.getItem(`sidebar_state::${projectId}`);
      return storedState ? JSON.parse(storedState) : ['Executive', 'Operations', 'Support'];
    } catch (error) {
      console.error("Failed to parse sidebar state:", error);
      return ['Executive', 'Operations', 'Support'];
    }
  };

  const [openDepartments, setOpenDepartments] = useState<string[]>(() => getInitialOpenState(selectedProjectId));
  
  useEffect(() => {
    setOpenDepartments(getInitialOpenState(selectedProjectId));
  }, [selectedProjectId]);
  ...
  const toggleDepartment = (name: string) => {
      ...
      localStorage.setItem(`sidebar_state::${selectedProjectId}`, JSON.stringify(newOpenDepartments));
  };
  ...
  const sidebarContent = (
    <div className="flex h-full w-72 flex-col border-r border-border bg-card">
        <div className="flex h-16 shrink-0 items-center border-b border-border px-6">
            <div className="p-2 rounded-lg bg-gradient-to-br from-[#4da3ff] to-[#6bd3ff]">
                <Icon name="brain-circuit" className="h-6 w-6 text-background" />
            </div>
            <span className="ml-3 text-lg font-semibold">AI Command</span>
        </div>
